// Conjuntos data - generated dynamically
const conjuntos = [];
const TOTAL_CONJUNTOS = 11; // Define el número total de conjuntos aquí

for (let i = 1; i <= TOTAL_CONJUNTOS; i++) {
    // ===== ASIGNACIÓN DE PRECIOS =====
    // Aquí puedes definir los precios para cada conjunto.
    let price;
    if (i === 1 || i === 5) {
        price = 450; // Precio para los conjuntos 1 y 5
    } else if (i === 2) {
        price = 600; // Precio para el conjunto 2
    } else if (i === 3) {
        price = 550; // Precio para el conjunto 3
    } else {
        price = 500; // Precio para los demás conjuntos
    }

    // Determinar las imágenes disponibles para cada conjunto
    let images = [];
    if (i >= 1 && i <= 11) {
        // Los conjuntos tienen 1 imagen cada uno
        images = [`Conjunto${i}.jpeg`];
    }

    conjuntos.push({
        id: `conjunto${i}`,
        name: `Conjunto ${i}`, // Nombre generado automáticamente
        images: images,
        price: price
    });
}
...
// Load conjuntos on page load
document.addEventListener('DOMContentLoaded', function () {
    console.log('🚀 Inicializando página de conjuntos...');
    loadConjuntos();
    setupModal();
    ...
    const headerVideo = document.querySelector('.conjuntos-header-videos video');
    if (headerVideo) {
...
// ===== SISTEMA DE CARGA DE CONJUNTOS =====
function loadConjuntos() {
    loadConjuntosBatch();
    setupLoadMore();
}

function loadConjuntosBatch() {
    const grid = document.getElementById('conjuntos-grid');
    if (!grid) {
        console.error('❌ No se encontró el contenedor de conjuntos');
        return;
    }

    const startIndex = loadedItems;
    const endIndex = Math.min(startIndex + ITEMS_PER_PAGE, conjuntos.length);
    ...
    setTimeout(() => {
        for (let i = startIndex; i < endIndex; i++) {
            const conjuntoCard = createConjuntoCard(conjuntos[i]);
            // Reemplazar skeleton con card real
            const skeleton = document.getElementById(`skeleton-${i}`);
            if (skeleton) {
                skeleton.replaceWith(conjuntoCard);
                observer.observe(conjuntoCard); // Observar la nueva tarjeta para la animación
            } else {
                grid.appendChild(conjuntoCard);
            }
        }

        loadedItems = endIndex;
        console.log(`✅ Cargados ${loadedItems} de ${conjuntos.length} conjuntos`);

        // Ocultar botón si todos los conjuntos están cargados
        const loadMoreBtn = document.getElementById('load-more-btn');
        if (loadMoreBtn && loadedItems >= conjuntos.length) {
            loadMoreBtn.style.display = 'none';
            loadMoreBtn.textContent = '🎉 Todos los conjuntos cargados';
        }
    }, 300);
}
...
function showLoadingSkeletons(grid, startIndex, endIndex) {
    for (let i = startIndex; i < endIndex; i++) {
        ...
        skeleton.className = 'conjunto-card conjunto-card-loading';
    ...
    if (loadMoreBtn) {
        loadMoreBtn.addEventListener('click', () => {
            ...
            setTimeout(() => {
                ...
                loadMoreBtn.innerHTML = '<i class="fas fa-plus"></i> Cargar Más Conjuntos';

                // Scroll suave a los nuevos elementos
                const newItems = document.querySelectorAll('.conjunto-card:not(.conjunto-card-loading)');
...
function createConjuntoCard(conjunto) {
    const card = document.createElement('div');
    card.className = 'conjunto-card animate-on-scroll';
    card.setAttribute('data-id', conjunto.id);

    const img = document.createElement('img');
    img.src = `../img/conjuntos/${conjunto.images[0]}`;
    img.alt = conjunto.name;
    img.loading = 'lazy';
    img.className = 'conjunto-image';
    ...
    info.className = 'conjunto-info';

    const name = document.createElement('div');
    name.className = 'conjunto-name';
    name.textContent = conjunto.name;

    const price = document.createElement('div');
    price.className = 'conjunto-price';
    price.innerHTML = `<i class="fas fa-tag"></i> ${conjunto.price} Bs`;
    ...
    // Event listeners
    card.addEventListener('click', () => openModal(conjunto));
...
function openModal(conjunto) {
    ...
    currentConjuntoImages = conjunto.images;
    currentImageIndex = 0;
    selectedSize = null;
    ...
    mainImage.alt = conjunto.name;
    modalMainImageContainer.appendChild(mainImage);

    // Usar el array de imágenes individual de la conjunto
    const orderedImages = conjunto.images;
    ...
    orderedImages.forEach((imageSrc, index) => {
        ...
        thumbnail.src = `../img/Conjuntos/${imageSrc}`;
        thumbnail.alt = `Vista ${index + 1} de ${conjunto.name}`;
    ...
    currentConjuntoImages = orderedImages;
    changeImage(0);

    // Actualizar información
    if (modalTitle) {
        modalTitle.textContent = conjunto.name;
    }

    if (modalDescription) {
        modalDescription.textContent = `Descubre los detalles de nuestro ${conjunto.name.toLowerCase()}. Un conjunto perfectamente coordinado de nuestra colección urbana, diseñado con materiales de primera calidad y estilo inconfundible.`;
    }

    if (modalPrice) {
        modalPrice.textContent = `${conjunto.price} Bs`;
    }
    ...
    console.log(`📱 Modal abierto: ${conjunto.name}`);
}

function changeImage(index) {
    if (currentConjuntoImages.length === 0) return;

    // Circular navigation
    if (index < 0) {
        index = currentConjuntoImages.length - 1;
    } else if (index >= currentConjuntoImages.length) {
        index = 0;
    }

    currentImageIndex = index;
    const imageSrc = currentConjuntoImages[index];

    const mainImage = document.querySelector('.modal-main-image .modal-image');
    if (mainImage) {
        mainImage.src = `../img/conjuntos/${imageSrc}`;
        mainImage.alt = `${currentConjunto.name} - Vista ${index + 1}`;
    ...
    if (imageIndicator && currentConjuntoImages.length > 1) {
        imageIndicator.innerHTML = `${index + 1}/${currentConjuntoImages.length}`;
    }
...
function updateWhatsAppLink() {
    ...
    if (whatsappLink && currentConjunto) {
        let message = `Hola, estoy interesado en el ${currentConjunto.name}`;
        if (selectedSize) {
        ...
        whatsappLink.addEventListener('click', function() {
            console.log(`WhatsApp clickeado: ${currentConjunto.name}, Talla: ${selectedSize || 'No seleccionada'}`);
            trackWhatsAppClick(currentConjunto.name, selectedSize);
        });
    }
}
...
function setupImageErrorHandling() {
    document.addEventListener('error', function(e) {
        if (e.target.tagName === 'IMG') {
            const img = e.target; // No es necesario cambiar aquí, ya que el path es 'conjuntos/' (minúscula)
            if (img.src.includes('Conjuntos/')) {
...
window.addEventListener('load', function() {
    ...
    if (loadedItems < conjuntos.length) {
        const nextBatch = conjuntos.slice(loadedItems, loadedItems + ITEMS_PER_PAGE);
        preloadImages(nextBatch);
    }
});

function preloadImages(conjuntosBatch) {
    conjuntosBatch.forEach(conjunto => {
        const img = new Image();
        img.src = `../img/Conjuntos/${conjunto.images[0]}`;
    });
}
  ...
  if (e.target.classList.contains('agregar-carrito') || e.target.closest('.agregar-carrito')) {
    ...
    if (modalTitle && modalPrice) {
      const product = {
        ...
        image: currentConjunto ? `../img/Conjuntos/${currentConjunto.images[0]}` : ''
      };
